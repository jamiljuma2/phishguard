...
function ReportModal({ open, onClose, report }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col relative overflow-hidden"
        ...
        <div className="flex-1 overflow-auto px-6 py-4">
          {report ? (
            <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                  ...
                  <ul className="text-sm text-slate-700 dark:text-slate-200 space-y-1">
                    <li>
                      ...
                      {report.input_type || report.type || "N/A"}
                    </li>
                    <li>
              ...
              {/* Suspicious Words */}
              {report.suspicious_words &&
                report.suspicious_words.length > 0 && (
                  <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                    <h3 className="font-semibold text-slate-900 dark:text-white mb-2">
                      Suspicious Words
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      {report.suspicious_words.map((word, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400"
                        >
                          {word}
                        </span>
                      ))}
              ...
              <div className="bg-slate-100 dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
                ...
                <pre className="text-xs text-slate-700 dark:text-slate-200 overflow-x-auto whitespace-pre-wrap break-words max-h-60">
